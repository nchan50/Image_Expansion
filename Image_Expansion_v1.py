...
def store_TM(vector, channel, orientation):
    file = f'Samples_v1\\TM\\{BGR[channel]}\\{orientation}\\transition.pkl'
    try:
...
def get_pairs(channel, orientation):
    file = f'Samples_v1\\TM\\{BGR[channel]}\\{orientation}\\pairs.pkl'
    if os.path.isfile(file):
        with open(file, 'rb') as f:
            return pickle.load(f)
    elif PRINT_LOG:
    ...
    for channel in [BLUE, GREEN, RED]:
        ...
        for side, values in LRTB.items():
            ...
            for i in range(UV[o].shape[1 - o]):
                ...
                # We use symmetric normalization to get TM_probs. We assume that unoriented mirror images exist, and so the paths are undirected: https://proceedings.neurips.cc/paper_files/paper/2006/file/4fa177df22864518b2d7818d4db5db2d-Paper.pdf#:~:text=Note%20also%20that%20the%20matrix%20F%20=,is%20non%2Dnegative%2C%20symmetric%20and%20F1%20=%201)
                TM_probs = [
