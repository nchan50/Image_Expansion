...
def expand_image(file, pixels, sides = 'L'):
	# side: Determines which sides to extend. left(L), right(R), top(T), bottom(B)
	...
	for channel in [BLUE, GREEN, RED]:
		...
		BGR_UV.append([np.round(U @ S_sqrt, DECIMALS), np.round(S_sqrt @ V, DECIMALS)]) # We consider the square root of diagonal entries to preserve the original image
		UV = BGR_UV[channel]
		for side, values in LRTB.items():
			...
			for i in range(UV[o].shape[1 - o]):
				...
				TM_probs = [TM.get_entry(nodeA, nodeB)/ (np.sum(TM.get_row(nodeA)) * np.sum(TM.get_column(nodeB))) ** 0.5 for nodeA, nodeB in best_pairs]
...
display_image(cv.merge(expand_image('pattern_5.png', [10, 4, 9, 2], 'TBLR')))